...
class EdgeGAN(nn.Module):
    ...
    def forward(self, x, z):
        ...
        self.true_image_c_output = self.classifier(images)
        self.fake_image_c_output = self.classifier(self.image_g_output)
    ...
    def train(self, train_dl, epochs=100, batch_size=64):
        ...
        for epoch in range(epochs):
            for data in train_dl:
                ...
                print("Epoch: [%2d/%2d], time: %4.4f, joint_dis_dloss: %.8f, joint_dis_gloss: %.8f"
                      % (epoch, self.config.epoch, time.time() - start_time, 2 * discriminator_err, generator_err))